...
**Output format (plain text / quick notes):**
```markdown
...
**Output format (Outlook-ready, formatted HTML):**

Markdown pastes into Outlook as plain, unstyled text. For a summary that keeps its formatting (headers, colors, table) when pasted into an Outlook message body, use the HTML version instead:

1. Open `meeting_summary_outlook.html` in a browser.
2. Fill in the bracketed placeholders (or have the content generated and dropped straight in).
3. Select all (Ctrl+A / Cmd+A) → Copy (Ctrl+C).
4. Paste directly into the Outlook compose box — it pastes as rich formatted content, not plain text.

The HTML template includes:
- A blue header bar with meeting title, date, duration, and attendees
- Sectioned headings for Agenda, Key Discussion Points, Decisions, Action Items, and Next Steps
- A styled table for action items (Task / Owner / Due Date)
- Inline CSS throughout (required for formatting to survive the copy-paste into email clients)

> File: `meeting_summary_outlook.html` (companion file to this template guide)
...
5. For meeting summaries that need to look polished in an email (colors, table, headers), use the HTML output format and copy-paste from the rendered `meeting_summary_outlook.html` file into Outlook instead of the plain markdown version.
