...
6. **HTML export:** Convert `_analysis/FINAL-REPORT.md` into a standalone `_analysis/FINAL-REPORT.html` so anyone can open it directly in a browser — no Markdown viewer or Mermaid plugin required. Try in this order, use whichever succeeds:
   - **Preferred:** if `pandoc` is available, run `pandoc _analysis/FINAL-REPORT.md -o _analysis/FINAL-REPORT.html --standalone --metadata title="Application Architecture Analysis" -c <basic CSS file you author for readable typography/table styling>`. This gives correct GFM table and image rendering with minimal effort.
   - **Fallback 1:** if `pandoc` isn't available but Python is, `pip install markdown --break-system-packages` (or use it if already installed) and convert via `python -m markdown -x tables -x fenced_code _analysis/FINAL-REPORT.md -f _analysis/FINAL-REPORT.html`, then wrap the output in a minimal HTML shell with `<head><style>...</style></head>` for readable fonts, table borders, and code block formatting.
   - **Fallback 2:** if neither tool is available, hand-convert the Markdown to HTML directly (headings → `<h1>`-`<h3>`, tables → `<table>`, fenced code → `<pre><code>`, images → `<img>`) wrapped in the same styled HTML shell. This guarantees a working browser-viewable file even with zero tooling.
   - **Image paths:** the SVG diagrams referenced in section 6 live under `_analysis/diagrams/`. Keep `_analysis/FINAL-REPORT.html` in the same `_analysis/` folder as the `diagrams/` subfolder so the relative `<img src="diagrams/...svg">` paths keep working. If the report needs to be portable as a single file (e.g. to email or attach), instead inline each SVG's raw markup directly into the HTML in place of the `<img>` tag.
   - Verify the HTML opens correctly (tables render as tables, diagrams render as images, headings are structured) before considering this batch done.
7. **Tell the user where things stand:** After each batch, briefly state which batch just completed and which is next, so progress is visible even though the full report isn't done yet.
...
- Follow the "Execution Strategy — Batch Processing" above for any repository large enough that the report would otherwise be truncated. The definition of "done" is: manifest fully checked off, `_analysis/FINAL-REPORT.md` exists with all 10 sections populated for every module, AND `_analysis/FINAL-REPORT.html` exists and opens correctly in a browser with tables and diagrams rendering — a partial report, or a Markdown-only deliverable, is not an acceptable final answer.
